...
function _request(opts, cb, data) {
    ...
    console.log('_request with options %s', util.inspect(options));
    try {
        var req = http.request(options, function (res) {
            ...
            res.on('error', function (err) {
                console.log('response error: %s', err.message);
                _cb(err);
            })
            res.on('end', function(){
                _cb();
...
module.exports = {
    ...
    init:function (cb) {
        var ls = child_process.spawn(ELASTIC_ROOT + '/elasticsearch', ['-f']);
        ls.on('exit', cb);
    ...
    search: function(query, cb){
        _request({
            ...
            path: '/noogle/message/_search?q=' + query
        }, cb);
    },
