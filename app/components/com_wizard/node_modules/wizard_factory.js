...
var _DEBUG = false;
console.log('nuby factory: %s', util.inspect(nuby_factory))
...
module.exports = {
    ...
    make_action:function (wizard, step, steps, controller) {
        ...
        var post_input_function = nuby_factory.method_factory.on_input(['if (next) {rs.go("/' + wizard.name + '/' + _prev_step(step, steps).name + '")',
            '} else if (prev) { rs.go("/' + wizard.name + '/' + _next_step(step, steps).name + '")',
            '} else { self.on_output(rs, {})',
            '}'], ['prev', 'next'], 'post');
        var callback = new nuby_factory.method_factory.Function_Def('', ['if (next) {rs.go("/' + wizard.name + '/' + _prev_step(step, steps).name + '")',
            '} else if (prev) {rs.go("/' + wizard.name + '/' + _next_step(step, steps).name + '")',
            '} else { self.on_output(rs, {})',
            '}'],
            ['err', 'state'])

        var post_process_function = nuby_factory.method_factory.on_process(['var self = this', 'this.models.wizard_state.set_state(' , callback, ')'], ['next', 'prev'], 'post');

        action.add_post('*/' + step.name, {
            on:false,
            validate:post_validate_function,
            input:post_input_function,
            process:post_process_function
        });
    ...
    make_index_action:function (wizard, first_step, controller) {
        ...
        var on_validate = nuby_factory.method_factory.on_validate(["rs.go(rs, '" + route_prefix + '/' + route + "');"], false, '')
        action.add_on('*', {
